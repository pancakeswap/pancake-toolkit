...
/**
 * Fetch Top15 Tokens traded on PancakeSwap v2, ordered by trading volume,
...
const getTokens = async () => {
  try {
      ...
      gql`
        query ($from: ISO8601DateTime, $till: ISO8601DateTime, $blacklist: [String!]) {
          ethereum(network: bsc) {
            dexTrades(
              options: { desc: "Total_USD", limit: 15 }
    ...
    console.error(`Error when fetching Top15 Tokens by volume for the past 30 days, error: ${error.message}`);
  }
};
...
const main = async () => {
  ...
  const tokenListPath = `${path.resolve()}/src/tokens/pancakeswap-top-15.json`;
